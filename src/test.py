...
def save_test_results(tensor, target, output_class, counter, test_result_save_dir):
    """
...
def test(model, testloader, device, test_result_save_dir):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(testloader), total=len(testloader)):
            ...
            save_test_results(
                image, labels, output_class, counter, test_result_save_dir
            )
...
if __name__ == "__main__":
    test_result_save_dir = os.path.join("outputs", "test_results")
    ...
    predictions_list, ground_truth_list, acc = test(
        model, test_loader, DEVICE, test_result_save_dir
    )
