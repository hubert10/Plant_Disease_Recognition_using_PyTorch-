...
def get_datasets(image_size):
    ...
    dataset_test = datasets.ImageFolder(
        TEST_DIR, transform=(get_test_transform(image_size))
    )
    return dataset_test
...
def show_cam(CAMs, image, labels, output_class, save_name):
    for i, cam in enumerate(CAMs):
        ...
        cv2.putText(
            image,
            f"Pred: {CLASS_NAMES[int(output_class)]}",
            (5, 55),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            color,
        ...
        result = heatmap / 255.0 * 0.5 + image * 0.5
        # cv2.imshow('CAM', result)
        # cv2.waitKey(0)
        result = np.array(result, dtype=np.float32)
        image_concat = cv2.hconcat([orig_image, result])
        cv2.imwrite(save_name, image_concat * 255.0)
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(testloader), total=len(testloader)):
            ...
            save_name = os.path.join(
                "..", "outputs", "cam_results", str(counter) + ".png"
            )
            # Show and save the results.
            show_cam(CAMs, image, labels, output_class, save_name)

    acc = 100.0 * (test_running_correct / len(testloader.dataset))
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        checkpoint = torch.load(
            os.path.join("outputs", "model.pth"), map_location=torch.device("cpu")
        )
    model = build_model(
        pretrained=False, fine_tune=False, num_classes=len(CLASS_NAMES)
    ).to(DEVICE)
    model.load_state_dict(checkpoint["model_state_dict"])

    predictions_list, ground_truth_list, acc = test(model, test_loader, DEVICE)
